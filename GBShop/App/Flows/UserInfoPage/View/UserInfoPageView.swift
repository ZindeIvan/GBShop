...
class UserInfoPageView : UIView {
    ...
    private(set) var changeDataButton : UIButton = {
        let button = UIButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("Change data", for: .normal)
        button.backgroundColor = .green
        button.setTitleColor(.black, for: .normal)
        button.setTitleColor(.green, for: .highlighted)
        button.layer.cornerRadius = 16.0
    ...
    private func setUpConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            changeDataButton.topAnchor.constraint(equalTo: bioTextField.bottomAnchor, constant: heightSpacing),
